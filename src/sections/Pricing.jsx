      ...
      <Element name="Pricing">
        <div className="container">
          <div
           ...
           border-s2 bg-s1/50 pb-40 pt-28 max-xl:max-w-4xl max-lg:border-none max-md:pb-32 max-md:pt-16">
            <h3
              className="h3 max-lg:h4 max-md:h5 z-3 relative mx-auto mb-14 max-w-lg text-center
            ...
            <div
             ...
             bg-s1/50 p-2 backdrop-blur-[6px] max-md:w-[310px]">
              <button onClick={() => setMonthly(true)} className={`pricing-head_btn ${monthly && "text-p4"}`}>
                Monthly
              </button>
              <button onClick={() => setMonthly(false)} className={`pricing-head_btn ${!monthly && "text-p4"}`}>
                Annual
              </button>
              <div
                className={`g4 rounded-14 before:h-100 pricing-head_btn_before 
              absolute left-2 top-2 h-[calc(100%-16px)] w-[calc(50%-8px)] overflow-hidden
               shadow-400 transition-transform duration-500 ${!monthly && "translate-x-full"}`}
              />
            </div>
          ...
          <div className="scroll-hide relative z-2 -mt-12 flex items-start max-xl:gap-5 max-xl:overflow-auto max-xl:pt-6">
            {plans.map((plan, index) => (
              <div
                ...
                <div className={`flex flex-col items-center ${index === 1 ? "pt-24" : "pt-12"}`}>
                  <div
                    className={`small-2 rounded-20 relative z-2 mx-auto mb-6 border-2 px-4 py-1.5 uppercased ${
                  ...
                  <div className="relative z-2 flex items-center justify-center">
                    <div className={`h-num flex items-start ${index === 1 ? "text-p3" : "text-p4"}`}>
                      $<CountUp start={plan.priceMonthly} end={!monthly ? plan.priceYearly : plan.priceMonthly} />
                    </div>
